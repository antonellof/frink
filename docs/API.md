...
| `n` (>1) | **Served on `/v1/completions` and `/v1/chat/completions`**: the prompt is prefilled once and the KV forked per choice, so `prompt_tokens` counts the prompt ONCE while `completion_tokens` sums. Each choice is parsed for tool calls and reasoning in its own right. Choice `i` samples from `seed + i`, so choice 0 is byte-identical to the `n = 1` answer at the same seed, and at `temperature 0` every choice is the same sequence because the sampler is deterministic. **Served on the paged store too**, by copy-on-write: the fork shares every page whose positions are all written and copies only the part-written tail, so a 6000-token prompt forked four ways prefills once and copies four pages rather than four prompts. The fork takes its generation pages up front, as admission does for the first choice, because the decode loop has nowhere to report a store that ran dry at token 300 of 400. Two refusals there, named apart because they send an operator to different places: a checkpoint that slides a window over its KV cannot fork at all (a slid page is recycled privately, and two sequences recycling out of one set would read each other's later positions), and a store with no free pages is retryable. **Served with `stream`**, INTERLEAVED: the choices are decoded a token at a time and each chunk carries its own `choices[].index`, with a `role` on the first chunk of each choice, one terminal chunk per choice and one `usage` block on the last. Each choice keeps its own reasoning split and tool-call parser, so two choices can be mid-marker in different places. The interleaved schedule turns the prompt-lookup drafter off for that request: a speculative round commits a block, and bursts of five tokens per choice are what interleaving exists to avoid. A server-side conversation stores CHOICE 0, because a transcript is one conversation. **501 naming the field** on the native `/completion`, which returns a single `content` |
| `cache_salt` | **Served**: names the caller's prefix-cache namespace. Both shared caches key on it -- the contiguous prefix cache and the response cache -- so two callers sending the same prompt under different salts cannot be served each other's answer or each other's cached prefix. Without it, they can, and the shared leading tokens are usually the system prompt. The string is **hashed** and only the hash is kept, so a heap dump does not carry what the caller namespaced by. An absent, empty or whitespace salt is the shared namespace, which is what every request got before the field existed. **Served on the paged store too**: a namespace is a whole radix tree there, not a synthetic prefix inside one, because the tree matches whole PAGES and prepending anything would shift every page boundary. The pages stay ONE pool, so eviction reaches every namespace, and it takes from each in turn rather than by a global LRU: each tree's clock ticks only on walks of that tree, so comparing two of them would order by how busy a tenant is rather than by age, and a global LRU would let one busy caller evict every quiet caller's prefix, which is the noisy-neighbour problem this field exists to bound |
| `prompt_logprobs` | **Served on `/v1/completions`**: one entry per PROMPT token, the first `null` because nothing predicted it, each later one giving the log-probability the model assigned the token that actually followed, with its 1-based `rank`, plus up to `N` alternatives. Scored from the **plain softmax** of the model's logits, not the sampler's filtered chain -- a prompt token was supplied, not drawn, so a truncated distribution would describe a choice that never happened. That is the point: a token no sampler would pick still gets its number. Above 5 is a **400**. **Served on the paged store too**: such a request DECLINES the prefix tree at admission and runs every position of its own prompt, because an adopted position was computed by another request and has no logit row. Reporting holes was the alternative and is worse, since a caller asked for a number per position; declining costs that request its prefix sharing, which is what scoring a prompt is paying for anyway. **501** on the other wires, which have no field for it |
...
| `allowed_token_ids`, `bad_words` | **Served on all three generation wires.** Both steer the draw rather than ending it, which is what separates them from `stop`: a stop string ENDS a generation once produced, these make sure it never is. They are one mask, applied in the same closure the grammar, JSON mode and the reasoning budget already share, and no mask there ever clears another's `-inf`, so the result is the intersection whichever runs first. `bad_words` is **tokenized**: what is forbidden is a word's LAST token, and only when the tokens before it are exactly what was just generated (upstream's `NoBadWordsLogitsProcessor` rule) -- masking every token of the word would forbid every word that merely starts the same way. A one-token word has an empty prefix and so is forbidden everywhere, which is the common case. The rule is about tokens, so it is exact only for the tokenization the model would have produced; a word spelled across a different boundary can still come out, and no logit-processor implementation of this field avoids that. An EMPTY `allowed_token_ids` is a **400**, not a 501: it asks for a draw from nothing, and honouring it would return a token that is an artefact of argmax over negative infinity. Both are keyed by the response cache, because they change which tokens come back |
| `echo` | **Served on `/v1/completions`**, which is the wire that returns a continuation of the prompt; **501** on the chat and native wires, which return a message. The prompt and the completion come back as ONE `text`, and with `logprobs` the parallel arrays cover both: the prompt's entries are the PLAIN softmax (a prompt token was supplied, not drawn), the completion's are the sampler's own distributions, the first entry is `null` because nothing predicted the first prompt token, and `text_offset` runs over the echoed string so a client can slice `text` with it. Echoing a truncated prompt reports the tokens that were KEPT, not the string that was sent |
| `truncate_prompt_tokens` | **Served on all three generation wires**: keeps the prompt's LAST `k` tokens, applied after the BOS is prepended and before anything is prefilled, so the truncated sequence is the only one that exists -- the KV, `usage.prompt_tokens`, the prefix cache and `echo` all see the prompt that was answered. Below 1 is a **400**, not a 501: the field is implemented, and keeping none of the prompt is not a request any server can serve. Upstream's `-1` ("the model's maximum") is not spelled that way here, because this deployment's ceiling is a setting rather than a property of the checkpoint; name the number |
| `skip_special_tokens` | **Served on all three generation wires.** `false` keeps the model's end-of-generation token in the returned text and counts it in `usage`; it still ENDS the answer, because the field is about what comes back rather than about when to stop. A caller-supplied `stop_token_ids` entry is NOT affected: they asked to stop ON it, which means before it, exactly as a stop STRING is excluded from the text it ends. `true` is the default and is what this server always did |
| `return_tokens_as_token_ids` | **Served on all three generation wires**: a REPORTED token is spelled `token_id:123` instead of its text -- the `tokens` array, the `top_logprobs` keys, and the chat wire's `content[].token`. It does not change the completion's `text`, which is the answer rather than a report about it; a caller who wants the ids of the answer asks `/v1/tokenize`. It exists because a piece is not a unique name: two ids can detokenize to the same string, and a `top_logprobs` map keyed by text silently loses one of them |
