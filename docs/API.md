...
| `n` (>1) | **Served on `/v1/completions`**: the prompt is prefilled once and the KV forked per choice, so `prompt_tokens` counts the prompt ONCE while `completion_tokens` sums. Choice `i` samples from `seed + i`, so choice 0 is byte-identical to the `n = 1` answer at the same seed, and at `temperature 0` every choice is the same sequence because the sampler is deterministic. **501 naming the field** on `/v1/chat/completions` and the native `/completion`, which have no `choices` array to return several answers in |
